...
class Adapter(nn.Module):  # 一个即插即用的微调transformer模块罢了
...
class LocalFeature_loss_cal(nn.Module):
    def __init__(self, feature_dim):
        super().__init__()
    ...
    def forward(self, low_features, high_features, text_features):
        ...
        variances_differ = low_variances - high_variances
        variances_differ = torch.clamp(variances_differ, min=0.0)
        loss = torch.mean(variances_differ)
        return loss
...
class Troika(nn.Module):
    ...
    def __init__(self, config, attributes, classes, offset):
        ...
        self.local_feature_add = LocalFeature_loss_cal(output_dim)
    ...
    def multi_stage_cross_attention(self, q, low_level_features, high_level_features):
        ...
        q_2 = self.stage2_global_fusion(q, high_level_features)
...
img_feature torch.Size([16, 257, 1024]) 
    ...
    def forward(self, batch, idx, return_features=False):
        ...
        for i_element in range(self.token_ids.shape[0]):
            ...
            cmt_text_features = idx_text_features  + self.lamda_2 * cmt_2.squeeze(1)
