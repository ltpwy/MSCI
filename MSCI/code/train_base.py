...
from model.model_factory_final import get_model
...
def train_model(model, optimizer, config, train_dataset, val_dataset, test_dataset):
    ...
    for i in range(config.epoch_start, config.epochs):
        ...
        for bid, batch in enumerate(train_dataloader):
            ...
            logits = model(batch, train_pairs, return_features=True)  # 训练时添加解耦损失

            loss = model.loss_calu(logits,batch)
        ...
        if (i + 1) % 1 == 0:
            ...
            torch.save(model.state_dict(), os.path.join(config.save_path, f"epoch_{i + 1}.pt"))
        ...
        if config.val_metric == 'best_loss' and val_result["loss"] < best_loss:
           # print('val_loss:', val_result["loss"])
           # print('best_loss:', best_loss)
            best_loss = val_result["loss"]
            best_epoch = i + 1
            torch.save(model.state_dict(), os.path.join(
                config.save_path, "best_model.pt"))
        this_epoch_result= val_result["AUC"]
        best_epoch_result= best_metric
        if config.val_metric != 'best_loss' and val_result["AUC"] > best_metric :
            best_metric = val_result["AUC"]
            best_epoch = i + 1
            torch.save(model.state_dict(), os.path.join(
                config.save_path, "best_model.pt"))
        print('best_epoch:', best_epoch)
        print('loss:',val_result["loss"])
        final_model_state = model.state_dict()
        if this_epoch_result > best_epoch_result:
    ...
    if config.save_final_model:
        torch.save(final_model_state, os.path.join(config.save_path, f'final_model_wy.pt'))
